...
const showActiveTheme = (theme) => {
    ...
    if (btnToActive) {
        ...
        activeThemeIcon.classList.remove(
            activeThemeIcon.getAttribute("data-href").slice(1)
        );
...
function handleThemeBtn(theme) {
    $(".theme-btn-main").text("");
    if (theme === "light") {
        $(".theme-btn-main").append(
            "<i class='far fa-sun-bright theme-icon-active' data-href='#fa-sun-bright'></i>"
        );
    } else if (theme === "dim") {
        $(".theme-btn-main").append(
            "<i class='far fa-moon-stars theme-icon-active' data-href='#fa-moon-stars'></i>"
        );
    } else if (theme === "dark") {
        $(".theme-btn-main").append(
